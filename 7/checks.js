...
lib.query(friends,lib.select('name', 'gender', 'email'),lib.filterIn('favoriteFruit', ['Яблоко', 'Картофель']),
    lib.select('email', 'gender'), lib.filterIn('favoriteFruit', ['Картофель']), lib.select('fuck'));

console.log("------------------------------------------------------");

lib.query(friends, lib.filterIn('gender', 'Мужской'), lib.filterIn('favoriteFruit', 'Яблоко'));
